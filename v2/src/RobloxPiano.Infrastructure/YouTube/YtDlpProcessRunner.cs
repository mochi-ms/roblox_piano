...
public class YtDlpProcessRunner : IYtDlpProcessRunner
{
        ...
        CancellationToken ct = default)
    {
        ...
        {
            return ProcessExecutionResult.Failure(-1, $"yt-dlp 실행 파일을 찾을 수 없습니다: {executablePath}");
        }
        ...
        try
        {
            if (!process.Start())
            {
                return ProcessExecutionResult.Failure(-1, "yt-dlp 프로세스를 시작할 수 없습니다.");
            }
            ...
            catch (OperationCanceledException)
            {
                ...
                {
                    return ProcessExecutionResult.TimedOut($"yt-dlp 프로세스가 시간 초과({timeout?.TotalSeconds}초)되었습니다.");
                }

                return ProcessExecutionResult.Cancelled("yt-dlp 프로세스가 취소되었습니다.");
            }
        ...
        {
            KillProcessTreeSafely(process);
            return ProcessExecutionResult.Failure(-1, $"yt-dlp 프로세스 실행 중 예외 발생: {ex.Message}");
        }
    }
